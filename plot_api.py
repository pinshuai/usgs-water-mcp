...
from water_data_api import get_water_data_values
...
def register_plot_tools(mcp: FastMCP):
    """Register plotting tools with the MCP server."""
    ...
    async def plot_usgs_overlay(
        ...
        # Fetch all years in parallel — one HTTP request per year
        async def _fetch_year(yr: str):
            ...
            ts = data["value"]["timeSeries"][0]
            raw_vals = ts["values"][0]["value"]
            pairs = sorted(  # sort by date to guard against any ordering quirks
                (
        ...
        # Pull site/variable metadata from the first successful result
        ...
        for i, (yr, (_, pairs)) in enumerate(zip(year_list, results)):
            ...
            # Remap to reference year for aligned x-axis
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    # Show the *real* date in hover, not the reference year
                    customdata=real_dates,
                    hovertemplate=(
                        f"<b>{yr} %{{customdata}}</b><br>%{{y:.0f}} {unit}"
                        "<extra></extra>"
                    ),
                )
            )
        ...
        if not output_path:
            fd, output_path = tempfile.mkstemp(
                suffix=".html", prefix="usgs_overlay_"
            )
            os.close(fd)
